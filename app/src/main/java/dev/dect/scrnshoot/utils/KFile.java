...
import dev.dect.scrnshoot.activity.viewer.AudioActivity;
import dev.dect.scrnshoot.activity.viewer.ImageActivity;
...
public class KFile {
    ...
    public static File generateNewEmptyScrnshootFile(Context ctx, KSettings ks) {
        ...
        final String fileName =
            formatFileId(fileId)
            + FILE_SEPARATOR
            + getDefaultScrnshootFileName(ctx)
            + FILE_SEPARATOR
    ...
    public static String getDefaultScrnshootFileName(Context ctx) {
    ...
    public static long getAppTotalFilesSize(Context ctx, boolean includeCache) {
        ...
        for(Scrnshoot scrnshoot : new DB(ctx).selectAllScrnshoots(true)) {
            if(scrnshoot.getFile().exists()) {
                size += scrnshoot.getSize();
