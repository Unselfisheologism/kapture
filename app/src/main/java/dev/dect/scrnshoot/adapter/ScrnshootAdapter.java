...
import dev.dect.scrnshoot.fragment.ScrnshootsFragment;
...
public class ScrnshootAdapter extends RecyclerView.Adapter<ScrnshootAdapter.MyViewHolder> implements Filterable {
    private final String TAG = ScrnshootAdapter.class.getSimpleName();
    ...
    private final Filter FILTER = new Filter() {
        ...
        protected FilterResults performFiltering(CharSequence input) {
            final ArrayList<Scrnshoot> filteredList = new ArrayList<>();
            ...
            if(input == null || input.toString().trim().isEmpty()) {
                ...
                for(Scrnshoot scrnshoot : INITIAL_LIST_KAPTURES) {
                    if(scrnshoot.getName().toLowerCase().contains(search)) {
                        filteredList.add(scrnshoot);
                    }
                }
            }
        ...
        protected void publishResults(CharSequence input, FilterResults results) {
            if(results.values instanceof ArrayList) {
                LIST_KAPTURES.clear();
                LIST_KAPTURES.addAll((ArrayList<Scrnshoot>) results.values);
    ...
    public ScrnshootAdapter(ArrayList<Scrnshoot> listScrnshoots) {
        ...
        this.LIST_KAPTURES = new ArrayList<>(listScrnshoots);
    ...
    public static class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public MyViewHolder(View view) {
            ...
            if(this.STYLE == ScrnshootsFragment.STYLE_LIST) {
    ...
    public static class ItemLookup extends ItemDetailsLookup<Long> {
        ...
        public ItemDetails<Long> getItemDetails(@NonNull MotionEvent event) {
            ...
            if(view != null) {
                ...
                if(viewHolder instanceof ScrnshootAdapter.MyViewHolder) {
                    return ((ScrnshootAdapter.MyViewHolder) viewHolder).getItemDetails();
                }
            }
            return null;
        }
    ...
    @NonNull
    @Override
    public ScrnshootAdapter.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        switch(KSharedPreferences.getAppSp(parent.getContext()).getInt(Constants.Sp.App.LAYOUT_MANAGER_STYLE, DefaultSettings.LAYOUT_MANAGER_STYLE)) {
            case ScrnshootsFragment.STYLE_GRID_BIG:
                return new MyViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_scrnshoot_grid_big, parent, false));

            case ScrnshootsFragment.STYLE_GRID_SMALL:
                return new MyViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.layout_scrnshoot_grid_small, parent, false));
    ...
    @Override
    public void onBindViewHolder(@NonNull ScrnshootAdapter.MyViewHolder holder, int position) {
        final Scrnshoot scrnshoot = LIST_KAPTURES.get(position);
        ...
        holder.EL_FROM.setVisibility(scrnshoot.isFromWatch() ? View.VISIBLE : View.GONE);
        ...
        if(scrnshoot.hasExtras()) {
            ...
            holder.EL_EXTRA.setOnClickListener((v) -> {
                if(scrnshoot.hasExtras()) {
                    new ExtraPopup(ctx, scrnshoot.getExtras(), null).show();
                } else {
        ...
        if(scrnshoot.hasScreenshots()) {
            ...
            holder.EL_SCREENSHOT.setOnClickListener((v) -> {
                if(scrnshoot.hasScreenshots()) {
                    new ScreenshotPopup(ctx, scrnshoot.getScreenshots(), null).show();
                } else {
        ...
        if(holder.STYLE != ScrnshootsFragment.STYLE_LIST) {
            ...
            try {
                ...
                extractor.setDataSource(scrnshoot.getLocation());
        ...
        TRACKER.addObserver(new SelectionTracker.SelectionObserver<Long>() {
            ...
            if(!TRACKER.getSelection().isEmpty()) {
                if(holder.EL_SELECTOR.getTag() == null) {
                    ...
                    final ValueAnimator in = ValueAnimator.ofInt(0, (int) ctx.getResources().getDimension(R.dimen.scrnshoot_list_select_icon));
                ...
                final ValueAnimator out = ValueAnimator.ofInt((int) ctx.getResources().getDimension(R.dimen.scrnshoot_list_select_icon), 0);
