...
import dev.dect.scrnshoot.notification.WifiShareNotification;
import dev.dect.scrnshoot.popup.DialogPopup;
import dev.dect.scrnshoot.popup.WiFiSharePopup;
...
public class WifiShare extends NanoHTTPD {
    ...
    private final ArrayList<Scrnshoot> KAPTURES;
    ...
    public WifiShare(Context ctx) {
        this(ctx, new DB(ctx).selectAllScrnshoots(true));
    }

    public WifiShare(Context ctx, ArrayList<Scrnshoot> scrnshoots) {
        super(PORT);

        this.CONTEXT = ctx;
        this.KAPTURES = scrnshoots;
        this.NOTIFICATION = new WifiShareNotification(ctx);

        for(Scrnshoot scrnshoot : scrnshoots) {
            LOCATIONS.add(scrnshoot.getLocation());
    ...
    public void start() {
        if(KAPTURES.isEmpty()) {
            Toast.makeText(CONTEXT, CONTEXT.getString(R.string.toast_error_no_scrnshoots_found), Toast.LENGTH_SHORT).show();
